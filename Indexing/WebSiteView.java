...
public WebSiteView(File dir, String topDir){

final String footer="<p/><a class=\"a\" href=\"../index.html\"> Parent Directory</a><p/>"
	+"<a class=\"a\" href=\""+topDir+"/index.html\"> To Start Page </a>"
	+"\n</body>\n</html>";
...
try{
	...
	for( File v : list){
		...
		if(v.isDirectory()) {
		if(!v.getName().toLowerCase().startsWith("images") 
				&& !v.getName().toLowerCase().startsWith("resources") )
			dirs.push(v.getName());
		}

		if( v.isFile() && !v.getName().equals("index.html") && v.getName().endsWith(".html") ) 
			fils.push(v.getName());
		if( v.isFile() &&( v.getName().endsWith(".txt") || v.getName().endsWith(".jar")
		||v.getName().endsWith(".xml")|| v.getName().endsWith(".ps") ||v.getName().endsWith(".pdf") ||v.getName().endsWith(".txml")) )
			fils.push(v.getName());
	}
...
/////////////////////////
	...
	for (String v : ds ) new WebSiteView(new File(dir,v), topDir);
}catch(Exception ee){}
     }
...
public static void main(String[] args) throws Exception{
	...
	new WebSiteView(new File(args[0]), args[1]);
